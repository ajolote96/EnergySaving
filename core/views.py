...
def Encender(request):
    try:
        Peticion = requests.get('http://192.168.0.17/encender')
...
def Apagar(request):
    try:
        Peticion = requests.get("http://192.168.0.17/apagar")
        if Peticion.status_code == 200:
            data = {
                'code': Peticion.status_code,
                'description': "Apagado"
...
def Estado(request):
    ...
    try:
        Peticion = requests.get("http://192.168.0.17/estado")
        if Peticion.status_code == 200:
            data = {
                'code': Peticion.status_code,
...
def RecivirData(request):
    try:
        if request.method == 'GET':
            ...
            #RegistroMinuto.objects.create(irms=irms, voltaje=voltaje, watts=watts, intencidadPico=intencidadPico, fecha = fecha, hora = fecha)
            print(str(fecha))
